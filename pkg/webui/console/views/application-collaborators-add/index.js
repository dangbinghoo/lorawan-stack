...
import style from './application-collaborators-add.styl'
...
export default class ApplicationCollaboratorAdd extends React.Component {
  ...
  async handleSubmit (values, { resetForm }) {
    ...
    const collaborator_ids = {
      [`${collaborator_type.value}_ids`]: {
        [`${collaborator_type.value}_id`]: collaborator_id,
      },
    }
    const collaborator = {
    ...
    try {
      ...
      dispatch(push(`/console/applications/${appId}/collaborators/${collaborator_id}`))
    } catch (error) {
      resetForm(values)
      this.setState({ error })
    }
