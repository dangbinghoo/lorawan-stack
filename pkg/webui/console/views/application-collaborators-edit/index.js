...
import { getApplicationCollaborator } from '../../store/actions/application'
import api from '../../api'

import style from './application-collaborators-edit.styl'
...
const m = defineMessages({
  ...
  modalWarning:
    'Are you sure you want to delete the {collaboratorId} Collaborator?',
})
...
@connect(function ({ collaborators, rights }, props) {
  ...
  const collaborator = collaborators.applications.collaborators
...
@bind
export default class ApplicationCollaboratorsEdit extends React.Component {
  ...
  componentDidMount () {
    ...
    dispatch(getApplicationCollaborator(appId))
  }
  ...
  render () {
    ...
    const { rightsItems, rightsValues } = rights.reduce(
      function (acc, right) {
        ...
        acc.rightsValues[right] = collaborator.rights.includes(right)
        ...
        <Row>
          <Col lg={8} md={12}>
            ...
            >
              <Message
                component="h4"
                content={sharedMessages.collaboratorDetails}
              />
              <Field
              ...
              <div className={style.submitBar}>
                ...
                <ModalButton
                  ...
                  modalData={{ message: m.modalWarning }}
